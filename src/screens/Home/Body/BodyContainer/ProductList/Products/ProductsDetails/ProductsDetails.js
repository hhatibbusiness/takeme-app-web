...
const ProductsDetails = ({currentProduct, productTypes, productType, setPopup, popup, lan, filter, setCurrentProduct, value, fetchProductTypeDetails, startFetchingProvidersProducts, resetProductData, resetProvidersFetchingPage, fetchProductDetails}) => {
    ...
    useEffect(() => {
        if(!currentProduct && productTypes?.length > 0) {
            const currentProductType = productTypes?.filter(p => p?.id == params.id)[0];
            setCurrentProduct(currentProductType);
        }
                        ...
                        imgUI && (
                            <div ref={productRef}  className={`ProductDetails__body ${imgLoaded ? 'ProductsDetails__visible' : 'ProductsDetails__hidden'}`}>
                                ...
                                <div ref={imgContainer} className="ProductDetails__img--container">
                                    <Img product={true} setError={setError} setHidden={setHidden} setLoaded={setLoaded} setImgLoaded={setImgLoaded} imgUrl={(currentProduct?.imagePath) || productDefaultImg}/>
