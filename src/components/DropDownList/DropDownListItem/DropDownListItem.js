...
const DropDownListItem = ({result, term, openPopup, changePopupProduct, changeCurrentProvider, openSearchPopup, setInputFocus, inputRef}) => {
    ...
    const selectAll = () => {
        const str = result?.productResponse?.name?.toLowerCase();
        const regex = /le/gi;
        const indices = [];
    ...
    const renderName = () => {
        var innerHTML = result?.productResponse?.name;
        var index = innerHTML?.indexOf(term);
        const occ = findAllOcc(result?.productResponse?.name, term);
        if(!term) {
            titleRef?.current.insertAdjacentHTML('beforeend', `<p className={'DropDownListItem__header--text-text'}>${result?.productResponse?.name}</p>`);
        } else {
            ...
            for(let i = 0; i < occ?.length; i++ ) {
                ...
                title.insertAdjacentHTML('beforeend', `<span class="DropDownListItem__header--text-highlight">${result?.productResponse?.name?.substring(occ[i], occ[i] + term?.length)}</span>`)
                if(i == occ?.length -1) {
        ...
        <div onClick={e => {
            ...
            <div className="DropDownListItem__container">
                ...
                <div className="DropDownListItem__prices">
                    {
                        result?.productResponse?.saleDetails && (
                            <div className="DropDownListItem__prices--sale">
                                <p className="DropDownListItem__prices--sale-paragraph"><span className={'DropDownListItem__prices--sale-message'}>{t('price')}</span><span className={'Product__details--sale-starts'}>{result?.productResponse?.saleDetails?.price}</span><span className={'Product__details--icon'}><i className="fa-solid fa-shekel-sign"></i></span></p>
                            </div>
                        )
                    }
                    {
                        result?.productResponse?.rentDetails && (
                            <div className="DropDownListItem__prices--sale">
                                <p className="DropDownListItem__prices--sale-paragraph"><span className={'DropDownListItem__prices--sale-message'}>{t('rentPrice')}</span><span className={'Product__details--sale-starts'}>{result?.productResponse?.rentDetails?.price}</span><span className={'Product__details--icon'}><i className="fa-solid fa-shekel-sign"></i></span></p>
                            </div>
                        )
                    }
                </div>
                {
                    result?.productResponse?.description?.text && (
                        <div className="DropDownListItem__desc">
                            <p>{result?.productResponse?.description?.text.slice(0, 80)}</p>
                        </div>
                    )
                }
            </div>
